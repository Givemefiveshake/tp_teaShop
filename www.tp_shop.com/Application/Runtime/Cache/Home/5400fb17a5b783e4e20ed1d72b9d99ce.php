				...
				<?php if($_SESSION['userInfo'] == null): ?><p class="not_login">
					...
					<?php else: ?>
						<p class="not_login">
							...
							<a href="<?php echo U('Order/pay');?>">用户中心</a><span class="spliter">|</span>
					...
					<li class="order_list">
						...
						<table class="all">
							...
							<?php if(is_array($orders)): $i = 0; $__LIST__ = $orders;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<?php switch($order['status']): case "1": ?><a href="" style='background: #5FB878;padding: 7px 12px;'>去支付</a><?php break;?>
										<?php case "2": ?>待发货<?php break;?>
										<?php case "3": ?><a href="" style='background: #5FB878;padding: 3px 5px;'>确认收货</a><?php break;?>
