                ...
                <div class="panel-body">
                    <form role="form" action="<?php echo U('edit');?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="exampleInputEmail1" name="name" value="<?php echo ($rows["name"]); ?>" placeholder="请输入名字">
                        </div>
                        <div>
                        <div class="form-group"  style="width: 50%; float: left">
                            <label for="exampleInputEmail1">商品分类</label>
                            <select class="form-control" name="goods_category_id">
                                <?php if(is_array($categories)): $i = 0; $__LIST__ = $categories;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category): $mod = ($i % 2 );++$i;?><option value="<?php echo ($category["id"]); ?>" <?php if($category['id'] == $rows['goods_category_id']): ?>selected<?php endif; ?>><?php echo str_repeat('&emsp;',($category['level']-1)*2); echo ($category["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                        <div class="form-group"  style="width: 50%; float: right">
                            <label for="exampleInputEmail1">商品品牌</label>
                            <select class="form-control" name="brand_id">
                                <?php if(is_array($brands)): $i = 0; $__LIST__ = $brands;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$brand): $mod = ($i % 2 );++$i;?><option value="<?php echo ($brand["id"]); ?>" <?php if($brand['id'] == $rows['brand_id']): ?>selected<?php endif; ?>><?php echo ($brand["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                        </div>
                        <div>
                            <div class="form-group" style="width: 25%; float: left">
                                <label >市场价</label>
                                <input type="text" class="form-control"  name="market_price" value="<?php echo ($rows["market_price"]); ?>" placeholder="请输入市场价">
                            </div>
                            <div class="form-group" style="width: 25%; float: left">
                                <label >本店价</label>
                                <input type="text" class="form-control"  name="shop_price" value="<?php echo ($rows["shop_price"]); ?>" placeholder="请输入本店价">
                            </div>
                            <div class="form-group" style="width: 25%; float: left">
                                <label >库存</label>
                                <input type="text" class="form-control"  name="stock" value="<?php echo ($rows["stock"]); ?>" placeholder="请输入库存">
                            </div>
                            <div class="form-group" style="width: 25%; float: left">
                                <label for="exampleInputEmail1">排序</label>
                                <input type="text" class="form-control" name="sort" value="<?php echo ($rows["sort"]); ?>" placeholder="请输入排序数字,越小越靠前">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">参与活动:&emsp;</label>
                            <?php if(is_array($promotions)): $i = 0; $__LIST__ = $promotions;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$promotion): $mod = ($i % 2 );++$i;?><input type="checkbox" name="prom_id[]" value="<?php echo ($promotion["id"]); ?>" <?php if(in_array($promotion['id'],$rows['prom_id'])): ?>checked<?php endif; ?>><?php echo ($promotion["prom_type"]); ?>&emsp;&emsp;<?php endforeach; endif; else: echo "" ;endif; ?>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">商品说明</label>
                            <textarea id="container" name="content"><?php echo ($rows["content"]); ?></textarea>
                        </div>
                        <img src="<?php echo ($rows["logo"]); ?>" width="100px" height="100px">
                        <div class="form-group">
                        ...
                        <div class="checkbox">
                            <label>
                                <input type="radio" name="is_on_sale" value="1"  <?php if($rows['is_on_sale'] == 1): ?>checked<?php endif; ?> /> 上架
                                <input type="radio" name="is_on_sale" value="0"  <?php if($rows['is_on_sale'] == 0): ?>checked<?php endif; ?> /> 下架
                            </label>
                        </div>
